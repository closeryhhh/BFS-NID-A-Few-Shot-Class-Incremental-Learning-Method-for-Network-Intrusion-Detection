...
for day_pcaps in filename_day_map.keys():
    ...
    if time_label_attacks:
            for attack in time_label_attacks:
                ...
                range_start_time, range_end_time, _, attacker_IPs = attack

                if _ not in flow_dict.keys():
                    continue
                ...
                for ip_pcap_file in tqdm(day_pcaps_path):
                    if len(flow_dict[BENIGN_TYPE]) == 0:
                        break

                    sub_root_save_path = os.path.join(root_save_path, BENIGN_TYPE)
                                ...
                                for (ts, pkt) in pkts_reader:
                                    ...
                                    if eth.type == dpkt.ethernet.ETH_TYPE_IP:
                                        ...
                                        try:
                                            ...
                                            continue
                                        if ip_src in attacker_IPs or ip_dst in attacker_IPs:
                                            continue
                                        ...
                                        if biflow_id in flows_maps.keys():
                                            ...
                                            if (ts - cur_biflow['begin_time']) >= timeout2 or (ts - last_seen_time) >= timeout:  # 持续时间 or 间隔时间
      
                                                save_biflow = flows_maps.pop(biflow_id)
                                                get_id = bening_id_map[BENIGN_TYPE]
                                                save_biflow_file_name = f'{BENIGN_TYPE}-{biflow_id}-{get_id}.pcap'
                                                bening_id_map[BENIGN_TYPE] += 1

                                                if get_id in flow_dict[BENIGN_TYPE]:
                                                    flow_dict[BENIGN_TYPE].remove(get_id)
